...
<template>
  <main class="flex h-dvh flex-col overflow-hidden">
    <header class="flex h-[68px] shrink-0 items-center border-b border-border bg-white px-8 max-[640px]:h-14 max-[640px]:px-4">
      <div class="flex items-center gap-3 text-base font-semibold tracking-[-0.025em] max-[640px]:text-sm">
        <div class="grid size-[34px] place-items-center rounded-[9px] bg-accent text-primary"><Workflow :size="21" /></div>
        <span>Workflow Studio</span>
      </div>
    </header>
    <section class="flex items-center justify-between gap-5 border-b border-border bg-white px-8 py-6 max-[640px]:flex-wrap max-[640px]:gap-4 max-[640px]:px-4 max-[640px]:py-[18px]" aria-label="Workflow tools">
      <div>
        <h1 class="m-0 text-[22px] font-semibold tracking-[-0.03em] max-[640px]:text-xl">Conversation workflow</h1>
      </div>
      <div class="flex items-center gap-2.5 max-[640px]:w-full">
        <Badge variant="secondary" class="mr-2 font-normal max-[640px]:mr-auto">{{ editableCount }} nodes</Badge
        ><Button
          variant="outline"
          size="icon"
        ...
        ><Button
          ...
          ><Plus />Create New Node</Button
        >
      </div>
    </section>
    <section class="relative min-h-0 flex-1 bg-canvas" aria-label="Workflow canvas">
      <div v-if="query.isPending.value" class="absolute inset-0 flex flex-col items-center justify-center gap-[30px]" aria-label="Loading workflow">
        <Skeleton class="h-28 w-64" /><Skeleton class="h-28 w-64" />
        <p class="text-[13px] text-muted-foreground">Opening your workflow…</p>
      </div>
      <div v-else-if="query.isError.value" class="mx-auto flex max-w-[420px] flex-col items-center justify-center gap-5 p-5">
        <Alert variant="destructive"
          ><AlertCircle /><AlertTitle>Couldn’t open the workflow</AlertTitle
          ><AlertDescription>{{ query.error.value?.message }}</AlertDescription></Alert
      ...
      />
      <div v-if="store.loaded && !store.graph.nodes.length" class="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-4">
        <Workflow :size="32" />
        <h2>Your next workflow starts here</h2>
        <p class="text-[13px] text-muted-foreground">Create a node, then connect it to build your flow.</p>
        <Button class="pointer-events-auto" @click="router.push('/nodes/new')"><Plus />Create New Node</Button>
      </div>
      <div v-if="invalidRoute" class="absolute right-5 top-5 flex w-[min(380px,calc(100%-40px))] flex-col gap-3">
        <Alert
      ...
      <div v-if="problem && !drawerOpen" class="absolute left-1/2 top-[18px] flex max-w-[90%] -translate-x-1/2 items-center gap-2 rounded-lg border border-border bg-white px-3.5 py-2.5 text-xs text-destructive" role="alert">
          <AlertCircle :size="16" />{{ problem
        }}<button class="border-0 bg-transparent" aria-label="Dismiss error" @click="problem = ''"><X :size="14" /></button>
      </div>
    </section>
    <NodeDrawer
      ref="drawer"
    ...
    <AlertDialog :open="discardOpen" @update:open="(value) => !value && resolveDiscard(false)"
      ><AlertDialogContent
        ><AlertDialogHeader
          ...
          ><AlertDialogDescription
            >Your edits haven’t been saved. Leaving this node will discard
            them.</AlertDialogDescription
          ></AlertDialogHeader
        ><AlertDialogFooter
          ><AlertDialogCancel @click="resolveDiscard(false)">Keep editing</AlertDialogCancel
          ><Button @click="resolveDiscard(true)">Discard changes</Button></AlertDialogFooter
        ></AlertDialogContent
      ></AlertDialog
    >
  </main>
